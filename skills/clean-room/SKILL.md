...
- `artifact validate` only checks JSON-schema shape; it cannot see whether a `public_surface:<spec_id>:<kind>:<name>` ref actually matches across files. After Agent 2 writes `implementation-plan.json`, run `clean-room-skill artifact check-coverage --spec <approved-spec-path>... --plan implementation-plan.json` before implementation starts. Before Agent 0 writes `clean-room-result.json`, run `clean-room-skill artifact check-coverage --task-manifest <path> --coverage-ledger coverage-ledger.json` — same join-checking logic the enforced runner uses for the completion gate, now callable standalone.
