...
const CLAUDE_PLUGIN_REGISTRY_REL_PATH = path.join('plugins', 'installed_plugins.json');
...
function expectedClaudePluginInstallPath(targetRoot, version = packageVersion()) {
  return path.join(targetRoot, 'plugins', 'cache', CLAUDE_PLUGIN_MARKETPLACE_NAME, CLAUDE_PLUGIN_NAME, version);
}

function claudePluginRegistryPath(targetRoot) {
  return path.join(targetRoot, CLAUDE_PLUGIN_REGISTRY_REL_PATH);
}
...
function readActiveClaudePluginEntry(targetRoot) {
  const registryPath = claudePluginRegistryPath(targetRoot);
  if (!fs.existsSync(registryPath)) {
    return { registryPath, entry: null, error: `active Claude plugin registry missing: ${registryPath}` };
  }
  let registry;
  try {
    registry = JSON.parse(fs.readFileSync(registryPath, 'utf8'));
  } catch (err) {
    return { registryPath, entry: null, error: `active Claude plugin registry is unreadable: ${err.message}` };
  }
  const entries = registry?.plugins?.[CLAUDE_PLUGIN_ID];
  const entry = Array.isArray(entries)
    ? (entries.find((item) => item?.scope === CLAUDE_PLUGIN_SCOPE) || entries[0])
    : null;
  return { registryPath, entry: entry || null, error: null };
}
...
function claudePluginEntryIssues(entry, expected = {}) {
  const expectedVersion = expected.version || packageVersion();
  const expectedInstallPath = normalizeOptionalPath(expected.installPath);
  const issues = [];
  if (!entry) {
    return [`active Claude plugin ${CLAUDE_PLUGIN_ID} is not installed`];
  }
  if (entry.enabled === false) {
    issues.push(`active Claude plugin ${CLAUDE_PLUGIN_ID} is disabled`);
  }
  if (entry.version !== expectedVersion) {
    issues.push(`active Claude plugin version is ${entry.version || '<unknown>'}, expected ${expectedVersion}`);
  }
  const activeInstallPath = normalizeOptionalPath(entry.installPath);
  if (expectedInstallPath && activeInstallPath && activeInstallPath !== expectedInstallPath) {
    issues.push(`active Claude plugin install path mismatch: ${activeInstallPath}, expected ${expectedInstallPath}`);
  } else if (expectedInstallPath && !activeInstallPath) {
    issues.push(`active Claude plugin install path is missing, expected ${expectedInstallPath}`);
  }
  return issues;
}

function claudeActivePluginStatus(targetRoot, manifest = null) {
  const state = readActiveClaudePluginEntry(targetRoot);
  const expectedInstallPath = manifest?.claude_plugin?.install_path ||
    expectedClaudePluginInstallPath(targetRoot);
  const issues = state.error
    ? [state.error]
    : claudePluginEntryIssues(state.entry, { installPath: expectedInstallPath });
  return {
...
function assertClaudeActivePluginStatus(targetRoot, manifest = null) {
  const status = claudeActivePluginStatus(targetRoot, manifest);
  if (!status.ok) {
    throw new Error(`Claude active plugin state is stale: ${status.issues.join('; ')}`);
  }
  return status;
}

function assertClaudePluginEntryCurrent(entry, expected = {}) {
  const issues = claudePluginEntryIssues(entry, expected);
  if (issues.length > 0) {
    throw new Error(`Claude plugin update did not activate ${CLAUDE_PLUGIN_ID}: ${issues.join('; ')}`);
  }
}
...
function ensureClaudeGlobalPlugin(layout, manifest, options, action) {
  ...
  assertClaudePluginEntryCurrent(pluginAfter, {
    installPath: expectedClaudePluginInstallPath(layout.targetRoot),
  });
  return claudePluginMetadata(manifest, {
