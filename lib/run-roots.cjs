...
const { fileHash, readJsonFile, sha256Bytes } = require('./fs-utils.cjs');
...
function stableJsonHash(value) {
  return sha256Bytes(Buffer.from(JSON.stringify(stableJsonValue(value)), 'utf8'));
}

function validateSiblingTaskManifestConsistency(taskManifestPath, manifest, roots) {
  const siblingPath = path.join(path.dirname(roots.contaminatedRoot), 'task-manifest.json');
  const siblingRealPath = realpathIfExists(siblingPath);
  if (!siblingRealPath) return;
  const activeRealPath = realpathIfExists(taskManifestPath);
  if (activeRealPath && siblingRealPath === activeRealPath) return;

  const siblingManifest = readJsonFile(siblingPath, null);
  if (stableJsonHash(siblingManifest) !== stableJsonHash(manifest)) {
    throw new Error(
      `root-level task-manifest.json conflicts with contaminated artifact manifest: ${siblingPath}; ` +
      `active manifest: ${taskManifestPath}. Use the contaminated artifact manifest for project-layout tasks, ` +
      'or move the stale root-level copy to quarantine before running.'
    );
  }
}
...
module.exports = {
  ...
  resolvePath,
  resolveRoots,
  validateSiblingTaskManifestConsistency,
