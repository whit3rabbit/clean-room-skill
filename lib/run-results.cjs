...
// Returns null when `artifactPath` exists as a regular file, otherwise a short
// reason. A directory where a file is expected is called out explicitly: that
// exact case (an empty `implementation-plan/` dir shadowing `implementation-plan.json`)
// otherwise reads as a plain "missing" and sends operators into a patch/rerun loop.
function completionArtifactProblem(artifactPath, artifactName) {
  if (!fs.existsSync(artifactPath)) {
    return `${artifactName} (missing)`;
  }
  if (!fs.statSync(artifactPath).isFile()) {
    return `${artifactName} (expected a file but found a directory at ${artifactPath}; remove the stray directory so the agent can write ${artifactName})`;
  }
  return null;
}
...
function validateTerminalCompletionArtifacts(roots) {
  ...
  // Aggregate every unmet artifact into a single error. Throwing on the first gap
  // surfaces blockers one rerun at a time, which is what drove the gate-by-gate loop.
  const problems = [];
  const readableFiles = [];
  for (const specPath of specPaths) {
    const problem = completionArtifactProblem(specPath, path.basename(specPath));
    if (problem) {
      problems.push(`clean behavior spec ${problem}`);
    } else {
      readableFiles.push(specPath);
    }
  }
  for (const [key, artifactName] of [
    ...
    const problem = completionArtifactProblem(artifactPath, artifactName);
    if (problem) {
      problems.push(`terminal clean artifact ${problem}`);
    } else {
      readableFiles.push(artifactPath);
    }
  }
  if (problems.length > 0) {
    throw new Error(`completion requires terminal clean artifact under CLEAN_ROOM_CLEAN_ROOTS; ${problems.length} unmet: ${problems.join('; ')}`);
  }
