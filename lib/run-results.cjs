...
function classifiedStageFailure(stageResult) {
  ...
  if (/OPENROUTER_API_KEY:\s*Set OPENROUTER_API_KEY|Set OPENROUTER_API_KEY for variant/i.test(output)) {
    return {
      summary: 'OpenRouter wrapper credentials are unavailable. Set OPENROUTER_API_KEY in the parent environment or run with --ccsilo [variant] so the controller preserves only the required wrapper credential env. Do not print the key while checking it.',
    };
  }
  if (/Not logged in\s*.*Please run \/login/i.test(output)) {
    return {
      summary: 'Claude auth is unavailable for the configured agent harness. For ccsilo/OpenRouter or other wrapper/API-key harnesses, verify CLEAN_ROOM_CLAUDE_EXECUTABLE, --agent-config-dir, and wrapper credentials; Claude /login applies only to OAuth-backed Claude sessions.',
    };
  }
