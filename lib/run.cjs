...
function validateCoveredBehaviorSpecs(roots, unitId) {
...
function validateCoverageLedgerIntegrity(manifest, roots, coverageLedger) {
  ...
  for (const unitId of coveredUnitIds) {
    validateCoveredBehaviorSpecs(roots, unitId);
  }
}
...
function validateTerminalCompletionArtifacts(roots) {
  ...
  for (const artifactName of ['implementation-report.json', 'qc-report.json']) {
    const artifactPath = path.join(roots.cleanRoot, artifactName);
  ...
  return withRunLock(roots.contaminatedRoot, options.dryRun, async () => {
    ...
    for (let offset = 0; offset < cap; offset += 1) {
      ...
      for (const stage of agentConfig.stages) {
        ...
        validateCleanRunContextReferences(options.python, roots);
        validateCoverageLedgerIntegrity(manifest, roots, readOptionalJson(path.join(roots.contaminatedRoot, 'coverage-ledger.json')));
