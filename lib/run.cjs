...
function inferTerminalResult(manifest, roots, selectedUnit) {
  ...
  const tickets = abstractTickets(report, qc, coverage, { abstract_delta_tickets: behaviorSpecOpenQuestionTickets(roots) });
...
function completeResultOrSpecDelta(manifest, roots, coverageStateValue = 'complete') {
  const tickets = behaviorSpecOpenQuestionTickets(roots);
  if (tickets.length > 0) {
