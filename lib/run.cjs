...
function validatePlanAndReportPublicSurfaceCoverage(plan, report, obligations) {
  ...
  for (const obligation of obligations) {
    const workItemIds = workItemsByRef.get(obligation.ref) || [];
  ...
  return withRunLock(roots.contaminatedRoot, options.dryRun, async () => {
    ...
    for (let offset = 0; offset < cap; offset += 1) {
      ...
      for (const stage of agentConfig.stages) {
        ...
        validateCleanRunContextReferences(options.python, roots);
        const stageCoverageLedger = readOptionalJson(path.join(roots.contaminatedRoot, 'coverage-ledger.json'));
    ...
    validateArtifacts(options.python, taskManifestPath, roots);
    validateCleanRunContextReferences(options.python, roots);
    const finalCoverageLedger = readOptionalJson(path.join(roots.contaminatedRoot, 'coverage-ledger.json'));
