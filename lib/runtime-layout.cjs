...
// Claude Code dynamic workflows (`.js` under <config>/workflows). Claude-only: no other runtime
// ships a Workflow() engine. Local-only so the workflow always installs alongside its front-door
// command wrapper (also local-only); a global workflow with no launchable front-door would bypass
// the skill's authorization discussion + confirmation gate.
const CLAUDE_WORKFLOWS = Object.freeze({ kind: 'workflows', source: 'workflows', destSubpath: 'workflows' });
  ...
  opencode: {
    ...
    artifacts: [
      ...
      },
      {
        ...
        destSubpath: 'plugins/clean-room.ts',
        hookModes: ['safe', 'strict'],
      },
      HOOKS,
    ],
  },
...
function resolveRuntimeLayout(runtime, scope, options = {}) {
  ...
  return {
    ...
    scope,
    targetRoot,
    hookRegistration: def.hookRegistration || 'none',
    supportsHookRegistration: !!def.hookRegistration,
