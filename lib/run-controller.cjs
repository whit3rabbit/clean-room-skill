...
const { applyCcsiloRunOptions } = require('./ccsilo.cjs');
...
async function runCleanRoom(options, context = {}) {
  ...
  validateSiblingTaskManifestConsistency(taskManifestPath, manifest, roots);
  ...
  return withImplementationRootLocks(roots.implementationRoots, options.dryRun, () => withRunLock(roots.contaminatedRoot, options.dryRun, async () => {
    ...
    if (options.dryRun) {
      const schemaSource = options.schemaDir
        ? 'schema override; safest path is bundled generated CLI schemas'
        : 'bundled generated CLI schemas';
      ...
      console.log(`clean-room run dry-run: schema dir: ${schemaDir} (${schemaSource})`);
      return {
    ...
    for (let offset = 0; offset < cap; offset += 1) {
      ...
      if (failedStage) {
        ...
        const classified = classifiedStageFailure(failedStage);
        if (classified) {
          console.log(`${failedStage.phase} failed: ${classified.summary}`);
        }
