...
const { readCcsiloVariantArg } = require('./ccsilo.cjs');
...
function printHelp() {
  console.log(`Usage: clean-room-skill [runtime] [scope] [options]
       ...
       clean-room-skill artifact <command> [options]
...
Commands:
  ...
  artifact            Create or validate canonical clean-room JSON artifacts
...
Options:
  ...
  --ccsilo [variant]   Install/status/update Claude in a ccsilo variant
