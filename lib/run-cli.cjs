...
Options:
  ...
  --ccsilo [variant]       Use a ccsilo Claude variant such as openrouter; implies
                           --agent-runtime claude, config dir, and wrapper executable
...
function parseRunArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '-h' || arg === '--help') {
      ...
      const parsed = readCcsiloVariantArg(argv, index);
      options.ccsilo = true;
