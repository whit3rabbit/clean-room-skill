...
- Map every exact-public-contract or behavior-compatible public surface obligation to at least one `implementation-plan.json` work item through `public_contract_refs`; do not replace a public command/API inventory with one generic dispatch work item unless every obligation ref is listed. Build each ref from the approved spec's own `spec_id`, never the source `unit_id` — see `skills/clean-room/references/SPEC-SCHEMA.md` (public-surface coverage ref rule) for the exact ref shape and an example; a mismatched ref will not join and the obligation stays unmapped.
- Every work item you create must end up recorded as completed or blocked in Agent 3's terminal report; if you leave a work item unscheduled, mark it blocked with a reason here rather than letting it disappear silently downstream.
