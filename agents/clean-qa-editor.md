...
- Verify public-surface inventory parity item by item. Every required `public_surface:<spec_id>:<kind>:<name>` ref must be covered by tests, mapped to a completed work item, and represented in terminal verification; passing test counts or broad command-dispatch coverage is not enough. `<spec_id>` is the spec's own `spec_id` field, not the source `unit_id` (see `skills/clean-room/references/SPEC-SCHEMA.md`) — a ref built from the wrong id will not match and the obligation stays unmapped even if a work item implements it.
- Every `implementation-plan.json` work item must appear in exactly one of `completed_work_items` or `blocked_work_items` in the terminal report, with a reason recorded for any blocked item. A plan work item present in neither list is a defect: report it as a finding or a blocked item rather than omitting it.
- Do not require a durable evidence-ledger entry as a precondition for `qc-report.json` `coverage_status: "complete"`. `evidence-ledger.json` is contaminated-side and this role is source-denied; evidence-backing is Agent 0's check after this report, not this role's.
